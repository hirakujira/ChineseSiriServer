...
class phonecallPlugin(Plugin):
    ...
    @register("en-US", u"(打給(.*))|(.*撥.*給(.*))|(.*電話給(.*))")
    def makeCall(self, speech, language, regex):

        personToCall = re.findall(ur"打給(.*)",speech)
        if personToCall == []:
            personToCall = re.findall(ur".*電話給(.*)",speech)
        if personToCall == []:
            personToCall = re.findall(ur".*撥.*給(.*)",speech)

        numberType = None
        numberType = self.getNumberTypeForName(numberType, language)
        persons = self.searchUserByName(personToCall[0])
        personToCall = None
        if len(persons) > 0:
            if len(persons) == 1:

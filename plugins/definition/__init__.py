...
class definition(Plugin):
    ...
    @register("en-US", u"(字典搜尋.*|搜尋字典.*|定義.*|搜尋定義.*|.*.的定義)")
    def define(self, speech, language, regMatched):

        if (language == "en-US"):
            
            if (speech.find(u'字典搜尋') == 0):
                speech = speech.replace(u'字典搜尋',' ',1)
            elif (speech.find(u"搜尋字典") == 0):
                speech = speech.replace(u"搜尋字典",' ',1)
            elif (speech.find(u"定義") == 0):
                speech = speech.replace(u"定義",' ',1)
            elif (speech.find(u"的定義") == 0):
                speech = speech.replace(u"的定義",' ',1)
            elif (speech.find(u"搜尋定義") == 0):
                speech = speech.replace(u"搜尋定義",' ',1)
